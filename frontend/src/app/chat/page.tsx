...
const ChatPageContent = () => {
  ...
  const needsSceneUpdate = useRef(false)
  ...
  // Initialize latestCodeBlock when first code block is received
  useEffect(() => {
    const blocks = messages.flatMap(msg => extractCodeBlocks(msg.content));
    const firstCodeBlock = blocks.find(block => block.isCode);
    if (firstCodeBlock && !latestCodeBlock) {
      setLatestCodeBlock({
  ...
  useEffect(() => {
    if (latestCodeBlock) {
      setCurrentLanguage(latestCodeBlock.language);
    }
  }, [latestCodeBlock]);

  const handleLanguageChange = (newLanguage: string, newCode: string) => {
    console.log('Language change handler called:', newLanguage);
    console.log('Previous code:', latestCodeBlock?.code);
    ...
    // Mark that we need a scene update
    needsSceneUpdate.current = true;

    // If we're in scene view, force a remount
    if (activeTab === "scene") {
      console.log('Forcing scene remount...');
      setShouldRerender(prev => prev + 1);
    }
  };

  // Effect to handle scene updates when switching to scene tab
  useEffect(() => {
    if (activeTab === "scene" && needsSceneUpdate.current) {
      console.log('Tab switched to scene, updating...');
      setShouldRerender(prev => prev + 1);
      needsSceneUpdate.current = false;
    }
  }, [activeTab]);
