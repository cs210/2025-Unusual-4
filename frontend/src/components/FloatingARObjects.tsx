...
import { CuboidIcon as Cube, Glasses, Scan } from 'lucide-react'

const AR_ICONS = [Cube, Glasses, Scan]

export function FloatingARObjects({ count = 5 }) {
  ...
  return (
    <div className="relative w-full h-full">
      {Array.from({ length: count }).map((_, i) => {
        const Icon = AR_ICONS[i % AR_ICONS.length]
        return (
          <motion.div
            key={i}
            className="absolute"
